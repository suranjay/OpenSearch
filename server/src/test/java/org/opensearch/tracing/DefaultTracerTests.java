...
public class DefaultTracerTests extends OpenSearchTestCase {
    ...
    public void testAddAttributes() {
        ArgumentCaptor<String> captorKeys = ArgumentCaptor.forClass(String.class);
        ArgumentCaptor<Long> longCaptorValues = ArgumentCaptor.forClass(Long.class);
        ArgumentCaptor<Double> doubleCaptorValues = ArgumentCaptor.forClass(Double.class);
        ArgumentCaptor<Boolean> booleanCaptorValues = ArgumentCaptor.forClass(Boolean.class);
        ...
        tracer.startSpan("foo", Level.INFO);
        tracer.addAttribute("stringKey", "stringValue");
        tracer.addAttribute("longKey", 10L);
        tracer.addAttribute("doubleKey", 2.0d);
        tracer.addAttribute("booleanKey", true);
        tracer.endSpan();
        ...
        verify(mockOtelSpan, times(7)).setAttribute(captorKeys.capture(), stringCaptorValues.capture());
        verify(mockOtelSpan, times(1)).setAttribute(captorKeys.capture(), longCaptorValues.capture());
        verify(mockOtelSpan, times(1)).setAttribute(captorKeys.capture(), doubleCaptorValues.capture());
        verify(mockOtelSpan, times(1)).setAttribute(captorKeys.capture(), booleanCaptorValues.capture());
        assertEquals("stringValue", stringCaptorValues.getValue());
        assertEquals(10L, longCaptorValues.getValue(), 0);
        assertEquals(2.0d, doubleCaptorValues.getValue(), 0);
        assertEquals(true, booleanCaptorValues.getValue());
